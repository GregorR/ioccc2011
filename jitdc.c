...
dat d) { S r("\01") Q(+)Q(-)Q(*)*M=-*M; R(_) Q(==)Q(<)Q(>)*M=!*M; R(!)
...
M++; S l[4](d); } } o*lib1[]={T,T,T,T,T,T,T}; o*lib2[]={T,ld,lP,lp,lx,lv,l1}; u
main(){ u i; k*b; dat d=st=J(sizeof(*st)); M=sk+9; S R=J(256*sizeof(k*));
for(i=0;i<256;i++)S R[i]=J(H); S m=n; S s=J(H); S l=(f*)lib1; S j=J(H); S
r=(f)re; for (i=0;m[i];i++)n[m[i]]=i; w=(k*)t; t(d); x[2]=w; y[2]=(k*)T-w+8; S
h=100; S l=(f*)lib2; S r=(f)T; j=e=
#if NOMMAP
malloc(H);
#else
